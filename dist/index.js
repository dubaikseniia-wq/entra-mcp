...
async function apiFetch(path, opts = {}, attempt = 0) {
    ...
    if (opts.auth) {
        if (!API_KEY)
            throw new ApiError(401, 'No ENTRA_API_KEY set', 'AUTH_ERROR', hintFor(401, ''));
...
function whoami() {
    if (!mePromise) {
        mePromise = apiFetch('/employer/api-keys/me', { auth: true })
            .then((r) => {
            const me = (r.data ?? r);
            if (!me.company?.id) {
...
server.registerTool('match_profile', {
    ...
    annotations: RO,
}, guarded(({ skills, role_hint, remote_only, limit }) => runMatchJobs({ skills, target_role: role_hint, remote_only, limit: limit ?? 8 })));
