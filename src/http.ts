...
function sendJson(res: Res, status: number, body: unknown, extra: Record<string, string> = {}): void {
...
const sendRpcError = (res: Res, status: number, code: number, message: string, extra: Record<string, string> = {}) =>
...
function retryAfterSeconds(ip: string): number | null {
  const now = Date.now();
...
function originHostProblem(req: Req): string | null {
  const host = req.headers.host;
  if (!host) return 'Missing Host header';
  if (ALLOWED_HOSTS.length && !ALLOWED_HOSTS.includes(host.replace(/:\d+$/, '').toLowerCase())) return 'Host not allowed';
  const origin = req.headers.origin;
  if (origin !== undefined && origin !== 'null') {
    try {
      const u = new URL(origin);
      if (u.protocol !== 'http:' && u.protocol !== 'https:') return 'Invalid Origin';
    } catch {
...
function bearerApiKey(req: Req): string {
...
function readBody(req: Req, res: Res): Promise<string | undefined> {
...
function rpcSummary(body: unknown): string {
...
async function handleMcp(req: Req, res: Res, note: (tag: string) => void): Promise<void> {
  if (req.method !== 'POST') {
  ...
  if (body === undefined) return sendRpcError(res, 400, -32600, 'Invalid Request: empty body');
  ...
  for (const [k, v] of Object.entries(CORS)) res.setHeader(k, v); // merged into the transport's writeHead
  try {
...
const httpServer = http.createServer(async (req, res) => {
  ...
  res.once('finish', () => {
    inFlight--;
    if (path !== '/healthz') console.log(`${req.method} ${path} ${res.statusCode} ${Date.now() - t0}ms${note ? ` ${note}` : ''}`);
  });
  res.once('close', () => {
    if (!res.writableFinished) inFlight--; // aborted before finish
  });
  try {
    if (shuttingDown) return sendRpcError(res, 503, -32000, 'Server is shutting down', { connection: 'close', 'retry-after': '2' });
    if (req.method === 'OPTIONS') {
      res.writeHead(204, CORS);
      return res.end();
    }
    const problem = originHostProblem(req);
    if (problem) return sendRpcError(res, 403, -32000, problem);
    ...
    if (req.method !== 'GET' && req.method !== 'HEAD') return sendRpcError(res, 405, -32000, 'Method not allowed', { allow: 'GET, HEAD, OPTIONS' });
    if (path === '/healthz') return sendJson(res, 200, { ok: true, version: VERSION, uptime_s: Math.round((Date.now() - STARTED_AT) / 1000) });
    if (path === '/') {
