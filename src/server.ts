...
// ---------- request context (which employer key, if any) ----------
// The key is never a module global: every McpServer built by createServer() owns one context, and each tool
// handler runs inside it via AsyncLocalStorage, so the hosted transport can serve many keys concurrently
// (one fresh server per HTTP request) while the stdio transport keeps its single process-wide key.
type Ctx = { apiKey: string; mePromise: Promise<Me> | null };
const ctxStore = new AsyncLocalStorage<Ctx>();
const currentKey = (): string => ctxStore.getStore()?.apiKey ?? '';
...
async function apiFetch<T = Json>(path: string, opts: FetchOpts = {}, attempt = 0): Promise<T> {
  ...
  if (opts.auth) {
    ...
    if (!key) throw new ApiError(401, 'No employer API key', 'AUTH_ERROR', hintFor(401, ''));
...
// =====================================================================
// ---------- EMPLOYER MODE (employer API key in the request context) ----------
// =====================================================================
...
function whoami(): Promise<Me> {
  ...
  if (!c || !c.apiKey) return Promise.reject(new ApiError(401, 'No employer API key', 'AUTH_ERROR', hintFor(401, '')));
  if (!c.mePromise) {
    c.mePromise = apiFetch<{ data?: Me }>('/employer/api-keys/me', { auth: true })
      .then((r) => {
...
export type CreateServerOptions = {
  /** Employer API key (entra_live_…). Omit / empty → candidate mode (8 read-only tools). */
  apiKey?: string;
};
...
export function createServer(opts: CreateServerOptions = {}): McpServer {
  ...
  const ctx: Ctx = { apiKey, mePromise: null };
  const tool = <A>(fn: (a: A) => Promise<ToolResult>) => {
    const safe = guarded(fn);
    return (a: A): Promise<ToolResult> => ctxStore.run(ctx, () => safe(a));
  };
  ...
  server.registerTool(
    ...
    tool(({ skills, role_hint, remote_only, limit }) =>
